...
func InsertExternal(database *sql.DB, query, lang string, items []ExternalResult) error {
	...
	stmt, err := tx.Prepare(`INSERT INTO external_results (query, language, title, url, snippet) VALUES (?, ?, ?, ?, ?)`)
	if err != nil {
		return err
	}
	defer stmt.Close()
	...
	for _, r := range items {
		_, err := stmt.Exec(query, lang, r.Title, r.URL, r.Snippet)
		if err != nil {
...
func GetExternal(database *sql.DB, query, lang string) ([]ExternalResult, error) {
	rows, err := database.Query(
		`SELECT title, url, snippet FROM external_results WHERE query = ? AND language = ?`,
		query, lang,
	)
	if err != nil {
		return nil, err
